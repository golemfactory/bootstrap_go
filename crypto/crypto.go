...
func (self *PrivateKey) EncryptPython(data []byte, pubKey PublicKey) ([]byte, error) {
	key := self.key
	ecdhKey, err := key.GetRawECDHKey(pubKey.key, 32)
	if err != nil {
...
func (self *PrivateKey) DecryptPython(raw []byte) ([]byte, error) {
	key := self.key
	cipher, err := elliptic.GetCipherByName("aes-128-ctr")
	if err != nil {
		return nil, errors.New("failed to get cipher: " + err.Error())
